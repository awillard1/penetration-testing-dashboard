...
async def list_activity(
    ...
    filters = []
    if engagement_id: filters.append(ActivityEvent.engagement_id == engagement_id)
    if event_type: filters.append(ActivityEvent.event_type == event_type)
    rows, _ = await list_all(session, ActivityEvent, filters, skip, limit, ActivityEvent.created_at.desc())
    return rows
