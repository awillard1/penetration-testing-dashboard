...
@router.get("")
async def list_runners(session: AsyncSession = Depends(get_session)):
...
async def create_runner(body: RunnerCreateBody, session: AsyncSession = Depends(get_session)):
...
async def update_runner(runner_id: str, body: RunnerUpdateBody, session: AsyncSession = Depends(get_session)):
...
async def revoke_runner_token(runner_id: str, session: AsyncSession = Depends(get_session)):
    row = await session.get(RunnerNode, runner_id)
    if not row:
        raise HTTPException(404, "Runner not found")
    token = secrets.token_urlsafe(32)
...
async def list_runner_jobs(runner_id: str, session: AsyncSession = Depends(get_session)):
    rows = (
